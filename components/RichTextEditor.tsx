...
export const RichTextEditor: React.FC<RichTextEditorProps> = ({ 
  ...
  // Sync external value to contentEditable SAFELY
  // Only update innerHTML if the component is NOT focused to prevent cursor jumping
  useEffect(() => {
    if (contentRef.current && !isSourceView) {
       const isActive = document.activeElement === contentRef.current;
       const currentHTML = contentRef.current.innerHTML;
       
       // Se não estiver focado, ou se estiver vazio, atualiza.
       // Se estiver focado, assume-se que o utilizador está a escrever e o DOM está mais atualizado que o prop 'value'
  ...
  const execCmd = (command: string, arg: string | undefined = undefined) => {
    ...
    // Garante que o foco volta para o editor se tiver sido perdido
  ...
  const ToolbarButton = ({ 
    ...
    return (
      <button
        type="button"
        onMouseDown={(e) => {
          e.preventDefault(); // CRUCIAL: Previne perda de foco do editor ao clicar no botão
          if (onClick) onClick();
        ...
        <div className="flex flex-wrap items-center gap-1 p-2 border-b border-indigo-100 bg-indigo-50/50 backdrop-blur-sm select-none">
           {!isSourceView ? (
             <>
                ...
                {/* Fonts */}
                <select 
                    className="h-7 text-xs bg-white/50 border border-indigo-100 rounded px-1 text-indigo-900 outline-none focus:border-indigo-400 cursor-pointer w-24"
                ...
                <div className="relative w-[28px] h-[28px] flex items-center justify-center hover:bg-indigo-200/50 rounded cursor-pointer group" title="Cor do Texto">
                    ...
                    <input 
                        ...
                        onInput={(e: any) => execCmd('foreColor', e.target.value)}
                ...
                <div className="relative w-[28px] h-[28px] flex items-center justify-center hover:bg-indigo-200/50 rounded cursor-pointer group" title="Cor de Fundo (Realce)">
                    ...
                    <input 
                        type="color" 
                        className="absolute inset-0 opacity-0 cursor-pointer w-full h-full" 
                        onInput={(e: any) => execCmd('hiliteColor', e.target.value)}
                        onMouseDown={(e) => e.preventDefault()}
                    />
                </div>
