...
export const MemberCard: React.FC<Props> = ({ member, onClick }) => {
    ...
    const SocialLink = ({ url, icon, colorClass, title }: { url: string, icon: React.ReactNode, colorClass: string, title: string }) => (
        <a 
            ...
            className={`p-2 rounded-full transition-colors flex items-center justify-center ${colorClass}`} 
            title={title}
        >
            {icon}
        </a>
    ...
    return (
        ...
        >
            ...
            <div className="text-sm text-indigo-600 mb-3 space-y-1 z-10 pointer-events-none h-6">
            ...
            {/* Actions / Socials (Stop Propagation) - APENAS SE EXISTIR LINK */}
            <div className="flex gap-2 mt-auto pt-4 border-t border-indigo-100 w-full justify-center z-10 flex-wrap min-h-[50px] items-center">
                ...
                {isVisible('twitter_url') && member.twitter_url && (
                    <SocialLink 
                        url={member.twitter_url}
                        title="Twitter / X"
                        colorClass="bg-gray-100 text-gray-900 hover:bg-gray-200"
                ...
                {isVisible('instagram_url') && member.instagram_url && (
                    <SocialLink 
                        url={member.instagram_url}
                        title="Instagram"
                        colorClass="bg-pink-100 text-pink-600 hover:bg-pink-200"
                ...
                {isVisible('facebook_url') && member.facebook_url && (
                    <SocialLink 
                ...
                {isVisible('tiktok_url') && member.tiktok_url && (
                    <SocialLink 
                        url={member.tiktok_url}
                        title="TikTok"
                        colorClass="bg-black/5 text-black hover:bg-black/10"
