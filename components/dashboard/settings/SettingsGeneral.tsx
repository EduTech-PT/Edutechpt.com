...
export const SettingsGeneral: React.FC<Props> = ({ dbVersion, profile, onNavigateToSql, onNavigateToDrive }) => {
    ...
    return (
        <GlassCard className="space-y-8 animate-in fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                     <div className="bg-white/40 p-6 rounded-xl border border-white/50">
                         ...
                         <div className="flex gap-2 items-center mb-3">
                             <input type="text" value={config.logoUrl || ''} onChange={e => setConfig({...config, logoUrl: e.target.value})} className="w-full p-2 rounded bg-white/50 border border-white/60 text-xs"/>
                             <label className={`px-3 py-2 bg-white text-indigo-600 border border-indigo-200 rounded font-bold cursor-pointer hover:bg-indigo-50 text-xs flex items-center gap-1 ${uploadingLogo ? 'opacity-50' : ''}`}>
                                 {uploadingLogo ? '...' : '📁'}
                                 <input type="file" className="hidden" accept="image/*" onChange={e => handleUpload(e, 'logo')} disabled={uploadingLogo} />
                             </label>
                         </div>
                         <div className="flex items-center justify-center py-4 bg-transparent rounded-lg border border-indigo-200 border-dashed h-32">
                             {config.logoUrl ? <img src={config.logoUrl} className="h-24 object-contain" /> : <span className="opacity-30 text-xs">Sem Logótipo</span>}
                         </div>
                     </div>

                     <div className="bg-white/40 p-6 rounded-xl border border-white/50">
                         <label className="block text-sm text-indigo-800 font-bold mb-3 uppercase tracking-wide">Favicon</label>
                         <div className="flex gap-2 items-center mb-3">
                             <input type="text" value={config.faviconUrl || ''} onChange={e => setConfig({...config, faviconUrl: e.target.value})} className="w-full p-2 rounded bg-white/50 border border-white/60 text-xs"/>
                             <label className={`px-3 py-2 bg-white text-indigo-600 border border-indigo-200 rounded font-bold cursor-pointer hover:bg-indigo-50 text-xs flex items-center gap-1 ${uploadingFavicon ? 'opacity-50' : ''}`}>
                                 {uploadingFavicon ? '...' : '📁'}
                         ...
                         <div className="flex items-center justify-center p-4 bg-transparent rounded-lg border border-indigo-200 border-dashed h-32">
                             {config.faviconUrl ? <img src={config.faviconUrl} className="h-8 w-8 object-contain" /> : <span className="opacity-30 text-xs">Sem Ícone</span>}
                         </div>
                     </div>
                </div>
            ...
            {/* LINKS RAPIDOS */}
            <div className="border-t border-indigo-100 pt-6">
                <h3 className="font-bold text-xl text-indigo-900 mb-4 flex items-center gap-2"><span>🔗</span> Links Rápidos</h3>
...
const QuickLinkCard = ({ label, url, onCopy }: any) => (
    <div className="bg-white/40 p-4 rounded-xl border border-white/50 text-xs">
        <label className="block font-bold text-indigo-800 mb-1">{label}</label>
        <div className="flex gap-2">
            <input type="text" readOnly value={url} className="w-full p-2 rounded bg-white/50 border border-white/60 text-indigo-600 font-mono outline-none" onClick={(e) => e.currentTarget.select()} />
            <button onClick={() => onCopy(url)} className="px-3 bg-white text-indigo-600 border border-indigo-200 rounded font-bold hover:bg-indigo-50">Copiar</button>
        </div>
    </div>
);

const VersionRow = ({ label, current, expected, onUpdate, status }: any) => (
    <div className="flex items-center justify-between p-4 bg-white/50 border border-white/60 rounded-xl">
        <div className="flex flex-col">
            <span className="text-sm font-bold text-indigo-900 uppercase tracking-wide opacity-70">{label}</span>
            <div className="flex items-center gap-2">
                <span className={`font-mono font-bold text-lg ${status === 'ok' ? 'text-green-600' : status === 'loading' ? 'text-indigo-500' : 'text-red-600'}`}>{current}</span>
                {status === 'error' && <span className="text-xs text-red-500 bg-red-50 px-2 py-0.5 rounded border border-red-100">Requer: {expected}</span>}
            </div>
        </div>
        <div>
            {status === 'ok' && <div className="flex items-center gap-2 px-4 py-2 bg-green-100 text-green-700 rounded-lg border border-green-200 shadow-sm"><span>✅</span><span className="font-bold text-sm hidden sm:inline">Atualizado</span></div>}
