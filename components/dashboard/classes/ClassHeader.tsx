    ...
    return (
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <div>
                ...
                <p className="text-sm text-indigo-700 opacity-80">Organize os alunos em grupos letivos.</p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                <div className="relative">
                    <select 
                        ...
                        className="w-full md:w-64 pl-4 pr-10 py-2 bg-white/60 border border-indigo-200 rounded-xl focus:ring-2 focus:ring-indigo-400 outline-none font-bold text-indigo-800 shadow-sm appearance-none cursor-pointer"
                    >
                        {courses.length === 0 && <option value="">A carregar cursos...</option>}
                        {courses.map(c => <option key={c.id} value={c.id}>{c.title}</option>)}
                    </select>
                    <span className="absolute right-3 top-2.5 text-indigo-500 pointer-events-none text-xs">▼</span>
                </div>
