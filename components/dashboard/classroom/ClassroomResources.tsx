        ...
        <div className="space-y-3 animate-in fade-in">
                    ...
                    {items.map((m: ClassMaterial) => {
                        ...
                        return (
                            <div key={m.id} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${isDone ? 'bg-green-50 border-green-200' : 'bg-white/60 border-indigo-100 hover:border-indigo-300'}`}>
                                ...
                                <a href={m.url} target="_blank" rel="noreferrer" className="p-2 bg-white rounded-lg text-indigo-600 shadow-sm hover:bg-indigo-50">↗</a>
                            </div>
                        );
                    })}
                </>
            ...
            {type === 'assessments' && items.map((a: ClassAssessment) => (
                <div key={a.id} className="bg-white/60 border border-indigo-100 p-5 rounded-xl flex flex-col md:flex-row justify-between gap-4">
                    ...
                    {a.resource_url && (
                        <a href={a.resource_url} target="_blank" rel="noreferrer" className="px-4 py-2 bg-indigo-100 text-indigo-700 rounded-lg font-bold hover:bg-indigo-200 transition-colors text-sm flex items-center gap-2 h-fit whitespace-nowrap">
